...
def get_X_y(data_file):
    ...
    with open(data_file) as fin:
        try:
            for FrontImageName, TopImageName, LeftRight1, LeftRight2, FrontBack1, FrontBack2, _, _, _, in csv.reader(
                    fin):
                if FrontImageName != 'FrontImageName':
                    X += [FrontImageName, TopImageName]
                    y += [float(LeftRight1), float(LeftRight2), float(FrontBack1), float(FrontBack2)]
        except:
            print("Done")
    return X, y


def process_image(path, joystick, augment, shape=(100, 100)):
